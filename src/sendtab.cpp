...
void MainWindow::inputComboTextChanged(int index) {
    ...
    auto balFmt = Settings::getZKCDisplayFormat(bal);
...
Tx MainWindow::createTxFromSendPage() {
    ...
    if (Settings::getInstance()->getAutoShield() && sendChangeToSapling) {
        auto saplingAddr = std::find_if(rpc->getAllZAddresses()->begin(), rpc->getAllZAddresses()->end(), [=](auto i) -> bool {
            // We're finding a sapling address that is not one of the To addresses, because zkcore doesn't allow duplicated addresses
...
bool MainWindow::confirmTx(Tx tx) {
    ...
    for (int i=0; i < tx.toAddrs.size(); i++) {
        ...
        // Add new Address widgets instead of the same one.
        {
            ...
            Amt->setText(Settings::getZKCDisplayFormat(toAddr.amount));
    ...
    // Add fees
    {
        ...
        minerFee->setText(Settings::getZKCDisplayFormat(tx.fee));
    ...
    QString tooltip = tr("Current balance      : ") +
        Settings::getZKCUSDDisplayFormat(rpc->getAllBalances()->value(tx.fromAddr));
    tooltip += "\n" + tr("Balance after this Tx: ") +
        Settings::getZKCUSDDisplayFormat(rpc->getAllBalances()->value(tx.fromAddr) - totalSpending);
